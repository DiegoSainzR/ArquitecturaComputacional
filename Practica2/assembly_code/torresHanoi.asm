...
.text
	addi s0, zero, 12		#s0 es para el numero de discos
	addi a0, zero, 0		#a0 es para		-Tecnicamente innecesario
	lui a1, %hi(torreOrigen)	#			(Load upper immediate)
	addi a1, a1, %lo(torreOrigen)
	slli t1, s0, 2			#t0 es para 		(Shift left immediate)
	add a3, a1, t1
	add a2, a3, t1
	...
	#a3 torre auxiliar
llenar:	
	beq s0, zero, iniciar		#(Branch =)
	slli t1, a0, 2
	add t1, t1, a1
	sw s0, 0(t1) 			#(Store word)
	addi a0, a0, 1
	addi s0, s0, -1
	j llenar
